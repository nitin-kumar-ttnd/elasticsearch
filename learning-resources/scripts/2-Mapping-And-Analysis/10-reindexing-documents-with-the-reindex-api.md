...
Changing how documents are mapped requires reindexing of docs
## Add new index with new mapping
Hit `GET /reviews/_mapping` and copy the `mappings` section in below json for new index.
```
PUT /reviews_new
{
  "mappings" : {
...
Retrieve docs from source index and index docs in new index.
```
POST /_reindex
{
  "source": {
